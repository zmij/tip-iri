...
class ParserTest :
        public ::testing::TestWithParam< ::std::pair< ::std::string, typename Parser::value_type > > {
protected:
    using parser_type = Parser;
    parser_type parser;
public:
    using result_type   = typename parser_type::value_type;
...
using Parse##test_name = ::parser_test::ParserTest< parser_name >; \
...
{ \
    ...
    EXPECT_TRUE(parser.done()) << "Not done with " << param.first; \
    EXPECT_FALSE(parser.failed()) << "Failed with " << param.first; \
    EXPECT_FALSE(parser.want_more()) << "Not finished with " << param.first; \
...
{ \
    ...
    EXPECT_TRUE(parser.failed()) << "Unexpectedly parsed '" << param.first \
            << "'. Value from parser " << parser.value(); \
